...
module.exports = {
	...
	token : function(appId, appSecret, authInfo, callback){
        const url = host+'/token?client_id='+appId+'&client_secret='+appSecret;
		
		console.log('Sending request to auth serv: POST ' + url +' body: '+ authInfo);
		request.post(url, {method: 'POST', uri: url, auth: {bearer: servAuth.token}, json: true, body: authInfo}, function(errors, response, body){
			if(errors) {
				console.log('error from request: ' + errors);
				if (errors.code == 'ECONNREFUSED')
					callback(errors, 500, '{\"error\": \"Service unavailable\"}' );
